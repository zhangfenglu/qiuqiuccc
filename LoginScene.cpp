...
//#include "SunNetPublic.h"  //大乱斗和兑换码增加的
//#include "ByRankLayer.h"  //七日的排行榜
//#include "ByFeedback.h"   //七日的排行榜
//#include "ByConvertHalo.h"  //七日的排行榜
//#include "ByLoginReward.h"   //七日的排行榜

//#include "FriendNetWork.h" //关系系统增加
//#include "json/document.h"  //关系系统增加
//#include "json/rapidjson.h"  //关系系统增加
//using namespace rapidjson; // 关系系统的命名空间

#include "HttpGetImg.h"     //合并代码用--0924--注册与组队
#include "TimeYuanZhengLayer.h"     //合并代码用--0924--注册与组队
#include "YuanZhengLayer.h"     //合并代码用--0924--注册与组队

//#include "StaticData.h"  //个人资料系统增加
//#include "paul/PersonalUI.h"  //个人资料系统增加
#include "GameVoice.h"
...
bool LoginLayer::init()
{
	if (!Layer::init())
		return false;
	rootNode = CSLoader::createNode("DaTingLayer.csb");   //合并代码用--0924--注册与组队 start====
...
void LoginLayer::initUI1()
{
	...
	auto  zuduidengdaiBg = (cocos2d::ui::ImageView*)seekNodeByName(rootNode, "zuduidengdaiBg"); //合并代码用--0924--注册与组队 start
	zuduidengdaiBg->setVisible(false); //合并代码用--0924--注册与组队 start====
	...
	if (strcmp(name.c_str(), "") != 0) {
		...
		CCUserDefault::sharedUserDefault()->setStringForKey("UserName", name.c_str());//合并代码用--0924--注册与组队 start====
	}
	...
	btnQianDao->addTouchEventListener([=](Ref*, cocos2d::ui::Widget::TouchEventType type)
	{
		...
		{
			GameVoice::getInstance()->playClickBtnVoive();
//			Director::getInstance()->getRunningScene()->addChild(ByLoginReward::create(), 10000);//七日的7日登录奖励界面
	...
	BtnKeFu->addTouchEventListener([=](Ref*, cocos2d::ui::Widget::TouchEventType type)
	{
		...
		{
			GameVoice::getInstance()->playClickBtnVoive();
//			SunNetPublic::getInstance()->addExchangeLayer(this);//打开兑换码界面。
		}
	});
	...
	btnNotice->addTouchEventListener([=](Ref*, cocos2d::ui::Widget::TouchEventType type)
	{
		...
		{
			GameVoice::getInstance()->playClickBtnVoive();
//			Director::getInstance()->getRunningScene()->addChild(ByFeedback::create(), 10000);//意见反馈界面
		}
	});
	...
	btnZhanDui->addTouchEventListener([=](Ref*, cocos2d::ui::Widget::TouchEventType type)
	{
		...
		{
			GameVoice::getInstance()->playClickBtnVoive();
//			Director::getInstance()->getRunningScene()->addChild(ByConvertHalo::create(), 10000);//兑换奖励界面
		}
	});
...
//			SunNetPublic::getInstance()->addExchangeLayer(this);//打开兑换码界面。
	...
	//圣衣  （圣衣按钮就是大乱斗按钮意思，这儿意思是点击按钮后，会触发什么功能的意思）
...
void LoginLayer::ResetName(Ref* pSender)
{
	...
	edbox->setText(Global::getInstance()->getString(randName));
	CCUserDefault::sharedUserDefault()->setStringForKey("UserName", Global::getInstance()->getString(randName));//合并代码用--0924--注册与组队 
}
...
void LoginLayer::RelationClick(cocos2d::Ref *pSender)   //打开关系界面
{
	//MainScene* main = dynamic_cast<MainScene*>(getParent());
...
//		relation->setVisible(false);
		...
		std::string data = "";//"socialtype=粉丝&playerid=" + ownPlayerId;
...
//			relation->showPlayerInfo(responseStr, 0);
		...
		//data = "socialtype=好友&playerid=" + ownPlayerId;
...
//			relation->showPlayerInfo(responseStr, 1);
		...
		//data = "socialtype=关注&playerid=" + ownPlayerId;
...
//	Director::getInstance()->getRunningScene()->addChild(ByRankLayer::create(), 100000);  //七日的排行榜的正式界面。
}
...
{
	//SunNetPublic::getInstance()->addApplyLayer(this);  //点击圣衣按钮，打开大乱斗界面
	log("===================================shengyi");
	auto timeYuanZhengLayer = TimeYuanZhengLayer::create();
	addChild(timeYuanZhengLayer, 100000);
}
			...
			if (result->valueInt == 1)
			{
				...
				if (account->child != NULL)
				{
					if (accout->valueString != "" && password->valueString != "")
					{
						...
						//加到这里
//						CCUserDefault::getInstance()->setStringForKey("playerid", playerid->valueString);//七日哥要求加载这儿的

//						StaticData::getInstance()->setPlayId(playerid->valueString);//zjf自己增加的，个人资料系统为了获取玩家id


						//  0922zjf屏蔽						
/*						// 2017-08-31					登陆成功后存储玩家的playerid，在请求玩家好友数据的时候会调用
						...
						// 2017-08-31					登陆成功后存储玩家的playerid，在请求玩家好友数据的时候会调用  end
*/
