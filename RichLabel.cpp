...
RichLabel * RichLabel::create(const char* string, const char* fontName, const int fontSize, Size labelSize, bool appendString, bool enableShadow)
{
...
bool RichLabel::init(const char* string, const char* fontName, const int fontSize, Size labelSize, bool appendString, bool enableShadow)
{
	...
	setColor(/*ccWHITE*/ cocos2d::Color3B::WHITE);
...
//void RichLabel::setColor(Color3B color)
//{
...
bool RichLabel::onTouchBegan(Touch *pTouch, Event *pEvent)
{
	Point point = convertToNodeSpace(pTouch->getLocation());
...
void RichLabel::onTouchEnded(Touch *pTouch, Event *pEvent)
{
	Point point = convertToNodeSpace(pTouch->getLocation());
...
void RichLabel::registerWithTouchDispatcherEx()
{
    //TouchDispatcher::getInstance()->addTargetedDelegate(this,PRI,false);
	//Director::getInstance()->getTouchDispatcher()->addTargetedDelegate(this, /*kCCMenuHandlerPriority - 20*/PRI, false);
}
	...
	if (str.length())
	{
		...
		if (width > mSize.width - mLabelOffsetX) // 字符串显示超过行边界，折断分成两行分别用两个label处理
		{
			...
			if (pos == 0) // 此行已无法完全显示一个字符,则从下一行创建label
			{
				...
				if (mLabelOffsetY < 0)
					setContentSize(Size(mSize.width, mSize.height - mLabelOffsetY + FONT_HEIGHT_GAP));
			}
			else
			{
		...
		else // 单行即可完全显示
		{
			...
			if (mEnableShadow == true)
			{
				...
				label->setPosition(Vec2(mLabelOffsetX + getAnchorPoint().x * label->getContentSize().width,
			...
			else
			{
                ...
                //label->setPosition(Vec2(mLabelOffsetX + getAnchorPoint().x * mSize.width,
			...
			if (mLinkTagFound)
			{
				Rect rect = Rect(mLabelOffsetX /*+ getAnchorPoint().x * label->getContentSize().width*/,
...
const char * RichLabel::parseEmoj(const char* string)
{
	...
	Sprite * _emoj = NULL;
	SpriteFrame *frame = SpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName(emoj.c_str());
	if (frame)
		_emoj = Sprite::createWithSpriteFrameName(emoj.c_str());
	else
		_emoj = Sprite::create(emoj.c_str());
	
    _emoj->setAnchorPoint(Vec2::ZERO);
	...
	_emoj->setPosition(Vec2(mLabelOffsetX, mLabelOffsetY/*-FONT_HEIGHT_GAP*/));
	_emoj->setScale((float)mFontSize/_emoj->getContentSize().height);
	addChild(_emoj);
	if (mLinkTagFound)
	{
		Rect rect = Rect(mLabelOffsetX+getAnchorPoint().x*_emoj->getContentSize().width,
...
GLubyte RichLabel::getOpacity(void) const
{
	Ref* child;
...
const Color3B& RichLabel::getColor(void) const
{
	return m_sColor;
}

void RichLabel::setColor(const Color3B& color3)
{
	m_sColor = color3;
