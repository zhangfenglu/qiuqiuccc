...
void RotateMenu::updatePositionWithAnimation(){
	...
	for (int i = 0; i < _items.size(); i++){
		...
		if (y == 270 && x < 600)
		{
			_items.at(i)->runAction(moveTo2);
		}
		else if (y == 270)
		{
			_items.at(i)->runAction(moveTo1);
		}
		else
...
void RotateMenu::rectify(bool forward){
	...
	while (angle>PI * 2)
		angle -= PI * 2;
	if (forward > 0)
		angle = ((int)((angle + getUnitAngle() / 6 * 2) / getUnitAngle()))*getUnitAngle();
	else
		angle = ((int)((angle + getUnitAngle() / 6) / getUnitAngle()))*getUnitAngle();
	setAngle(angle);
}
